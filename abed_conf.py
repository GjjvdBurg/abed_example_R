...
DATASETS = [
    ("dataset_%i_train" % i, "dataset_%i_test" % i) for i in range(1, 11)
]
...
PARAMS = {
    "OLS": {},
    "Lasso": {"alpha": [pow(2, x) for x in range(-8, 9, 2)]},
    "Ridge": {"alpha": [pow(2, x) for x in range(-8, 9, 2)]},
}

COMMANDS = {
    "OLS": (
        "Rscript --no-save --slave {execdir}/ols.R "
        "{datadir}/{train_dataset}.RData {datadir}/{test_dataset}.RData"
    ),
    "Lasso": (
        "Rscript --no-save --slave {execdir}/lasso.R "
        "{datadir}/{train_dataset}.RData {datadir}/{test_dataset}.RData "
        "{cv_seed} {alpha}"
    ),
    "Ridge": (
        "Rscript --no-save --slave {execdir}/ridge.R "
        "{datadir}/{train_dataset}.RData {datadir}/{test_dataset}.RData "
        "{cv_seed} {alpha}"
    ),
}
...
METRICS = {
    "MSE": {"metric": sklearn.metrics.mean_squared_error, "best": min},
    "MAE": {"metric": sklearn.metrics.mean_absolute_error, "best": min},
}
